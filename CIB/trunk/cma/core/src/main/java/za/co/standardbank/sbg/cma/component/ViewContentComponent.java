...
public class ViewContentComponent extends AbstractSBSAContentComponent {

	private static Category LOG = Category.getInstance(ViewContentComponent.class);

	private static final long serialVersionUID = 1L;
	public static final String ATTR_NAME_DATA = "vgnExtTemplatingComponentData";
	private static final String ATTR_NAME_CACHE_STRATEGY = "vgnExtTemplatingComponentCacheStrategy";

	public String getData() throws ApplicationException {
		String data = (String) getAttributeValue(ATTR_NAME_DATA);
		return data;
	}

	public long getComponentDefaultTTL() {
		return SysUtil.getConfigValLong("default.view.component.cache.cacheLifeTime", 300000L);
	}
	...
	public String createCacheKey(RenderedManagedObjectCacheKey key) throws ApplicationException {
		...
		try {
			keyGen = CacheKeyGeneratorFactory.getInstance().getCacheKeyGenerator(cacheStrategy);
		} catch (Exception e) {
			throw new CmaRuntimeException("Failed to create cache key for component", e);
		}
		sb.append(keyGen.createCacheKey(key, this));

		// Now add pagination if existing
		String vgnNextStartIndex = key.getRequestContext().getParameter("vgnNextStartIndex");
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Successfully created View Component cache key: " + sb.toString());
		}
