		...
		this.each(function(){
			...
			switch (c.type) {
				...
				case '1click2fame':
					...
					break;
				case 'vidavee':
					h += 'tribeca.vidavee.com/publish/vidavee/playerv3/vFlasher_debug.swf/d=' + c.videoId;
					break
			}
			e.flash({
				swf: 'http://' + h,
...
(function($, flash){
	...
	$[flash] = (function(){
		var flashVersion = '0,0,0', Plugin = navigator.plugins['Shockwave Flash'] || ActiveXObject;
		...
		return {
			...
			hasVersion: function(version){
				var versionCompare = this.version, major = 'major', minor = 'minor', release = 'release', version = (/string|number/.test(typeof version)) ? version.toString().split('.') : version || [0, 0, 0], version = [version[major] || version[0] || versionCompare[major], version[minor] || version[1] || versionCompare[minor], version[release] || version[2] || versionCompare[release]];
			...
			create: function(obj){
				if (!$[flash].available || expressInstallIsActive || !typeof obj == 'object' || !obj.swf) { return false} 
				...
				return '<object ' + (createAttrs(obj.attrs)) + ($[flash].activeX ? ' classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">' + '<param name="movie" value="' + obj.swf + '" />' : ' type="application/x-shockwave-flash" data="' + obj.swf + '">') + (createParams(obj.params)) + '</object>'
			}
		}
	}());
	$.fn[flash] = function(args){
		if (typeof args == 'object') {
			...
			this.find('object').andSelf().filter('object').each(function(){
				var elem = this, jsInteractionTimeoutMs = 'jsInteractionTimeoutMs';
