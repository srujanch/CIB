( function($) {// bodyguard function
	$(document).ready(function() {
		var $scrollingDiv = $('.leftNav2ColSmlRightBar.maxi .colRight .float'),
		padLeft = '6px',
		padRight = '6px',
	...
	//	$('#scroller a').wrapInner('<span class="label">');
		...
		$('ul.promoBanners li:last').addClass('last'); $('ul.promoBanners li:first').addClass('first');//banners for home page only
		...
		$('.headerStanlib .site_global, .headerStanlib .site_selector, .headerStanlib #footerNavBar,.headerStanlib #footerContentBar').remove();
		$('ul#scroller').fadeIn('fast');//jumpToLink
	...
	//	$('form.applicationsMenu').on('click','input.go',function(e){
		$('.internetBanking select.appMenuSelect').change(function(e) {
			$('.internetBanking select.appMenuSelect').val($(this).val());
		});
		$('.internetBanking input.go').click(function(e) {
			e.preventDefault();
			var targetURL = $('.appMenuSelect option:selected').attr('value'),
			targetType = $('.appMenuSelect option:selected').attr('label');
			if(targetURL !=='ignore'){
				switch(targetType){
				case 'blank': //Opens the linked document in a new window or tab
					alert('You have clicked a link that will redirect you in a new window/tab to the following URL \n \n'+ targetURL);
					window.open(targetURL);
					break;
				default: //default same window action if no target specified
					window.location = targetURL;
					break;
				}
			}else{alert('Please select an option from the list of Online applications');}
		});
		$('.styleSelectorx a').click(function(e){
...
//scrolling div for search bar on deals and news pages affects only $('.leftNav2ColSmlRightBar.maxi .colRight') because of class float 
	...
	//		$('#debugging_status').html('topFrameOff:'+topFrameOff+' | topFrameH:'+topFrameH+' | scrollHeight:'+scrollHeight+' | maxY:'+maxY);
		...
		$('ul#topNavMenu li a').on('mouseover',function(e){
			var $this=$(this),
	...
	//		$('#debugging_status').html($('.active'));
				$('#mask').addClass('topNavMenu').css({'width':maskWidth,'height':maskHeight,'top':-62});//Set height and width to mask to fill up the whole screen
...
//				$('#topMenuSub').empty();
				if ($this.next('div.navContent').length>0){
					...
					$('#mask').removeClass('topNavMenu').fadeOut('medium');
				}
			}
		});
		...
		$('#mask.topNavMenu').click(function () {//if mask is clicked
			clearNav();
			$('#topMenuSub').slideUp('slow', function(e){$('#topMenuSub').remove();});
			$(this).removeClass('topNavMenu').fadeOut('medium');
		});
		...
		$('ul.countryContacts h3, ul.accordian h3').wrapInner('<a href="#" class="region"><span class="label">&nbsp;</span></a>');
		$('ul.countryContacts .region, ul.accordian .region').append('<span class="arrowDn">&nbsp;</span>');
		...
		$('ul.accordian').on('click','.region',function(e){
			e.preventDefault();
			$(this).children('.arrowDn').toggleClass('arrowUp');
