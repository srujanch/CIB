( function($) {// bodyguard function
	...
	////////////////////DEBUGGING SCRIPT//////////////////////
		$(document).mousemove(function(e){
			if($('#status').length==0){
//				$('#page').prepend('<div id="status" style="position:fixed; left:0; top:0; z-index:5000; color:#fff; background-color:#000; opacity:0.5;padding:15px;"></div>');
			}
//			$('#status').html(e.pageX +', '+ e.pageY);
	...
	////////////////////DEBUGGING SCRIPT END//////////////////////
		$('#page').on('mouseover','a[rel=tooltip]',function(e) {//Select all anchor tag with rel set to tooltip
			var tip, ttWidth, ttHeight, ttLeftPos, ttTopPos;
			...
			if($(this).parents('#dropMenu').length>0){
				ttLeftPos=0;
				ttTopPos=-($('#tooltip').outerHeight()+8);
			}else{
				ttLeftPos=e.pageX-45;//-$(this).offset().left;
				ttTopPos=e.pageY-($('#tooltip').outerHeight()+18);//-$(this).offset().top;
			}
			if(width){
			...
			$('#tooltip').css('left', ttLeftPos).css('top', ttTopPos);//.css('position','absolute' );
...
//			if($(this).parents('#dropMenu').length>0){
//				ttLeftPos=-45;
//				ttTopPos=-($('#tooltip').outerHeight()+18);
//			}else{
//				ttLeftPos=e.pageX-45;//-$(this).offset().left;
//				ttTopPos=e.pageY-($('#tooltip').outerHeight()+18);//-$(this).offset().top;
//			}
//			$('#tooltip').css('left', ttLeftPos).css('top', ttTopPos);//.css('position','absolute' );
		...
		$('.internetBanking input.go').click(function(e) {
			...
			if(targetURL !==''){
				switch(targetType){
					case 'blank': //Opens the linked document in a new window or tab
						alert('You have clicked a link that will open in a new window/tab to the following URL \n \n'+ targetURL);
						window.open(targetURL,'linkedPageSBSA');
						break;
			...
			}else{alert('Please select an option from the list that you want to navigate to');}
		});
...
//scrolling div for search bar on deals and news pages affects only $('.leftNav2ColSmlRightBar.maxi .colRight') because of class float 
	...
	//		$('#debugging_status').html($('.active'));
		...
		$('ul.accordian li.active .region span.arrowDn').addClass('arrowUp');
		...
		$('ul.countryContacts ul li').on('mouseover',function(e){
			$(this).addClass('hover');
		}).on('mouseout',function(e){
			$('ul.countryContacts ul li').removeClass('hover');
		});
		...
		$('ul.accordian .region').live('click',function(e){
			e.preventDefault();
			if($(this).parents('ul').hasClass('toggle')){
			//	console.log($(this).parents('ul'));
				$(this).parents('ul').find('.arrowDn').removeClass('arrowUp');
...
// Country Scroller
	...
	//		$('.scrollDot').remove();
		...
		$('a.modal').live('click',function(e) {
			...
			contAreaH,closeH,
		//	dimensions = $(this).attr('rev'),
			targetFile = $(this).attr('href'),
			maskHeight = $(document).height(),//Get the screen height and width
			maskWidth = $(window).width(),
			winH = $(window).height(),		//Get the window height and width
			winW = $(window).width(),
			activWinH=$(window).height() - 60;
			
				if ($(window).width() > $('#pageContent').width()){
				//	console.log('condW1');
					activWinW=$('#pageContent').width() - 200;
				}else{
					activWinW=$(window).width() - 200;
				}
		...
		//	alert(dimensions);
			...
			$('#'+id+' .contentArea').load(targetFile, function(e){
				closeH = 10+$('#'+id+' a.close').height();
//				contAreaH = $('#'+id+' .contentArea').height();
//				console.log("conth"+contAreaH+" ::: WinH"+winH+" ::: CloseH"+closeH+" ::: activWinW"+activWinW+" ::: activWinH"+activWinH);
				$('#'+id+' .contentArea').css('height',$('#'+id).height()-closeH);
//				if (contAreaH+closeH > winH){
//					console.log('condH1');
//					$('#'+id+' .contentArea').css('height',$('#'+id+' .contentArea').height()-closeH);
//				}else{
//					console.log('conH2');
//					$('#'+id).animate({top:100},function(e){
//					$('#'+id+' .contentArea').animate({height:$('#'+id).height()-closeH});
//					});
//				}
			});//.css('height',$('#'+id+' .contentArea').height()-$('#'+id+' a.close').height());
		});
		...
		//	alert($(this).attr('rel'));
			...
			$('#'+id).css('height',  activWinH).css('width',  activWinW).css('top',  winH/2-$('#'+id).height()/2).css('left', winW/2-$('#'+id).width()/2).fadeIn(2000);		//transition effect
			$('#'+id+' .contentArea').css('height',$('#'+id).height()-$('#'+id+' a.close').height()).html('<iframe width="100%" scrolling="auto" height="100%" frameborder="0" src="'+targetFile+'" name="contentPopup"></iframe>');
		});
		...
		$('ul.bodyTabs a').click(function (e) {
			if($(this).attr('rel') !== undefined){
				e.preventDefault();
				$('ul.bodyTabs li').removeClass('activeTab');
				$(this).parent('li').addClass('activeTab');
				var $divId = $(this).attr('rel'),
					$hrefDest = $(this).attr('href'); 	
		...
		$('a.advSearchExpander').click(function(e){
			e.preventDefault();
			$(this).parents('fieldset').toggleClass('closedState');
		//	console.log($(this).parents('fieldset').find('.arrowDn.advSearchExpander').length);
			if($(this).parents('fieldset').find('.arrowDn.advSearchExpander').length > 0){
				$('.arrowDn.advSearchExpander').removeClass('arrowDn').addClass('arrowUp');
			}else{
				$('.arrowUp.advSearchExpander').removeClass('arrowUp').addClass('arrowDn');	
			}
		});
		$('#pageContent').on('click','ul.pagination a',function (e) {
			e.preventDefault();
			var $divId = $(this).attr('rel');
				$parentId = $(this).parents('.tabBody').attr('id');
				$hrefDest = $(this).attr('href'); 
			if ($('#'+$parentId+'_pagination_01').length == 0){
				$('#'+$parentId+'.tabBody').wrapInner('<div id="'+$parentId+'_pagination_01" class="paginationItem">');
			}
			$('#'+$parentId+' .paginationItem').fadeOut('medium');
			if($('#'+$parentId+'_pagination_' + $divId).length==0){
				$.get($hrefDest, function(data){
					$('#'+$parentId).append(data);
					$targetParent = $(data).find('div:first').parent().attr('id');
					$('#'+$targetParent).wrap('<div id="'+$parentId+'_pagination_'+$divId+'" class="paginationItem">');
				});
			}else{
				$('#'+$parentId+'_pagination_' + $divId).fadeIn('medium')
			}
		});
	});// bodyguard function
}) (jQuery);
