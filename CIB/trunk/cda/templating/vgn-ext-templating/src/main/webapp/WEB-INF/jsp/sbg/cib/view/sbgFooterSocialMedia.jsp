...
<c:if test="${not empty content && not empty content.SBG_RELATED_LINKS}">
	...
	<div id="socialMedia" class="portlet">
		...
		<c:forEach items="${sortedLinks}" var="relatedLink">
			...
			<li><a href="${itemURL}" class="${overLayClass}" target="${linkTarget}"${relAtt}${titleAtt} onclick="return trackSocialMediaClick('${relatedLink.linkText}')"><span class="${relatedLink.linkClassName}">&nbsp;</span> ${relatedLink.linkText}</a></li>
		</c:forEach>
		</ul>
	</div>
</c:if>