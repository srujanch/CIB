...
Date provided: 21/02/2012
Copyright 1996-2012 Adobe, Inc. All Rights Reserved
More info available at http://www.omniture.com */

/* Specify the Report Suite ID(s) to track here */
var s_account="accstandardbankdev"
...
s.linkInternalFilters="javascript:,standardbank.co.za,sbic.co.za"
...
s.useCommerce=false
s.varUsed="prop14"
s.eventList="" //Abandon,Success,Error for commerce vars only
...
function s_doPlugins(s) {
	...
	//Previous Page
	s.prop15 = s.getPreviousValue(s.pageName,'gpv_p9','');
	...
	if(typeof s.prop10 != 'undefined') {
		if(s.prop10 != '') {
			s.eVar10 = s.prop10;
			s.eVar30=s.crossVisitParticipation(s.prop10,'s_ev30','30','5','>');
	...
	//Time Parting
	s.prop18=s.eVar18=s.getTimeParting('','+2');

	//New vs. Repeat Visitor
	s.prop25=s.eVar19=s.getNewRepeat();

	//External Campaign Tracking
	s.campaign=s.getValOnce(s.getQueryParam('cid'),'s_campaign',0);

	//Internal Campaign Tracking
	s.eVar11=s.getValOnce(s.getQueryParam('intcmp'),'s_intcampaign',0);
	if (s.eVar11 != "")
	{
		s.eVar29 = "D=c15";
	}
	...
	if (s.campaign != "")
	{
		s.prop17 = s.campaign + ":" + "D=pageName";
	} else {
		s.prop17 = "D=pageName";
	}

	//Channel pathing
	s.prop19 = "D=ch" + ":" + "D=pageName";
	...
	if (CheckEvents('event6'))
	{
		s.ttc='start';
	} else if (CheckEvents('event7'))
	{
		s.ttc='stop';
	}
	s.eVar28=s.getTimeToComplete(s.ttc,'ttc',0);
...
//Check if an event is set in the s.events variable
function CheckEvents(eventname){ 
  arrEvents = s.events.split(',');
  var Exists = false; 
  for (var i = 0; i < arrEvents.length; i++){ 
	if (eventname == arrEvents[i])
	{
		Exists = true;
	}
  } 
...
s.trackingServer="accstandardbank.112.2o7.net"
