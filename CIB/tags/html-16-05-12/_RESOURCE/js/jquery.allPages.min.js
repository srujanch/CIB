...
// new jquery.mousewheel.min.js 3.06
...
var userBrowser = BrowserDetect.browser + ' ' + BrowserDetect.version,
	expDays = 30, // number of days the cookie should last
	BDpath = location.protocol + '//' + location.hostname+'/web';
...
//	((domain === null) ? "" : ("; domain=" + domain)) +
	((secure === true) ? "; secure" : "");
	cookiedetails = name + "=" + escape(value) +
	((expires === null) ? "" : ("; expires=" + expires.toGMTString())) +
//	((path == null) ? "" : ("; path=" + path)) +
	"; path=/" +
//	((domain === null) ? "" : ("; domain=" + domain)) +
...
//	document.cookie = name='something'; expires='toGMTString()'; path='';
...
function checkBrowserCompatibility(isBrowserCondition) {
//alert(BrowserDetect.browser);
var browserMessage,
	notSupported=false,
	lastSuppFirefoxVersion=6,
	lastSuppIeVersion=7,
	upgradeOpt='<p class="upgradeOpt"><b>Upgrade options:</b></p><ul class="upgrade"><li><a href="http://www.google.com/chrome" target="_blank">Upgrade Google Chrome &raquo;</a></li><li><a href="http://windows.microsoft.com/en-US/windows/products" target="_blank">Upgrade Internet explorer &raquo;</a></li><li><a href="http://www.mozilla-europe.org/en/firefox/" target="_blank">Upgrade Firefox  &raquo;</a></li></ul>';
	if(! isBrowserCondition){
		browserMessage = '<p class="notice">You are viewing <b>' + location.protocol + '//' + location.hostname + ' with ' + userBrowser + '</b> and may experience limited functionality and security.</p><a href="#" class="readMore">More info &raquo;</a><div class="moreInfo"><p>To receive the most effective security and protection from threats like phishing, hacking and identity theft, please ensure that your browser software is up to date or that you have a version that supports the latest Internet security protection. </p><p>We cannot guarantee that our website will accommodate Internet browsers which are out of date and do not comply with our stringent security policies.</p>'+upgradeOpt+'</div>';
	}else{
		browserMessage = '<p class="notice">You are attempting to <b>view a page with ' + userBrowser + '</b> and will experience limited functionality and security.</p><a href="#" class="readMore">More info &raquo;</a><div class="moreInfo"><p>'+userBrowser +' does not support the latest technology required for modern secure browsing standards, please ensure that your browser software is up to date or that you have a version that supports the latest web standards </p><p>We cannot guarantee that the our website will accommodate Internet browsers which are out of date, therefore you may not be able to proceed unless you update your browser.</p><p> An upgrade to Firefox will not remove your ' + userBrowser + ' installation.</p>'+upgradeOpt+'</div>';
	}

	if (BrowserDetect.browser === 'Firefox'){
		if (BrowserDetect.version <= lastSuppFirefoxVersion) {
			notSupported=true;
			runCookie();
		}
	} else if(BrowserDetect.browser === 'Internet Explorer'){
		if (BrowserDetect.version <= lastSuppIeVersion) {
			notSupported=true;
			runCookie();
		//	var isBrowserCondition=BrowserDetect.version;
 //alert(BrowserDetect.version);
		}
	} else if(BrowserDetect.browser === 'Chrome'){
	}
		if (navigator.cookieEnabled == false ){//|| navigator.cookieEnabled === 0) {
			$('#page').before('<div class="topNotification"><div class="message"><p class="notice">It appears that you either have cookies disabled or are not using a browser that utilizes cookies.  <b>This website requires cookies to function correctly.</b></p></div><a href="#" class="closeNotification">X<\/a><br class="clearBoth"><\/div>').fadeIn('medium');
		}
	function runCookie(){
		var count = GetCookie('count');
		// alert(isBrowserCondition);
		if ((count === null) && (count <= 1) || (notSupported == true)) {// (isBrowserCondition==='isIE7')) {
			count=1;
			count++;
			SetCookie('count', count, exp);
			$('#page').before('<div class="topNotification">').slideDown('medium');
			$('.topNotification').append('<h1>Older browser detected:</h1><div class="message">'+browserMessage+'</div><a href="#" class="closeNotification">X<\/a><br class="clearBoth"><\/div>').slideDown('medium');
		//	alert(count);
			return false;
			//window.open(page, "", windowprops);
		} else {
			//alert('has seen cookie already '+ count)
		}
	}
}
//$('#page').before('<div id="features">');
//$('#features').html(function(){return jQuery.support});
( function($) {
